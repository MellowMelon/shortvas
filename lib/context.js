...
// In addition to basic proxying, the transform attribute is modified according
// to Transform.process.
var proxyContextTransformMethod = function (proto, name, alias) {
  proto[alias || name] = function () {
    this.transform = Transform.process(this.transform, name, arguments);
    var ret = this.context[name].apply(this.context, arguments);
    return typeof ret === "undefined" ? this : ret;
  };
};
...
ctxMethods.forEach(function (name) {
  if (["save", "restore"].indexOf(name) > -1) {
    // Manually implemented below
  } else if (ctxTransformMethods.indexOf(name) > -1) {
...
sCtxProto.restore = function () {
  ...
  this.transform = this.transformStack.pop() || this.transform;
  this.context.restore();
  return this;
};
// Aliases
sCtxProto.pop = sCtxProto.restore;
sCtxProto.r = sCtxProto.restore;
...
sCtxProto.blockTransform = function (f) {
  var origTransform = this.getTransform();
  f();
  this.setTransform(origTransform);
  return this;
};
// Aliases
sCtxProto.blockT = sCtxProto.blockTransform;

sCtxProto.with = function (props, f) {
  var origValues = {};
  for (var k in props) {
  ...
  for (var k in origValues) {
    if (origValues.hasOwnProperty(k)) {
      this[k] = origValues[k];
    }
  }
  return this;
...
sCtxProto.blank = function (color) {
  this.blockT(function () {
    this.resetT().bp().rect(0, 0, this.width, this.height).fill(color);
  }.bind(this));
  return this;
};
...
sCtxProto.clear = function () {
  this.blockT(function () {
    this.resetT().clearRect(0, 0, this.width, this.height);
  }.bind(this));
  return this;
};

sCtxProto.beginPath = function () {
  this.context.beginPath();
  return new ShortvasPath(this);
};
// Aliases
sCtxProto.bp = sCtxProto.beginPath;
